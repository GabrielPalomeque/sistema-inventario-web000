...
# --- CONFIGURACIÓN DE LA SUCURSAL (COLUMNAS CORRECTAS) ---
COLUMNAS_TIENDA = {
    "MI STORE CENTER": 4,  
    "SUCURSAL_1": 5,       
    "SUCURSAL_2": 6,       
...
try:
    ...
    st.error(f"Error al conectar con Google Sheets. Revisa tus credenciales o el nombre del archivo.\n{e}")
    st.stop()
        ...
        if st.session_state.ultimo_recibo_html != "":
            ...
            with col_izq:
                ...
                if prod_sel:
                    ...
                    s2 = fila_datos[COLUMNAS_TIENDA["SUCURSAL_1"]-1] if len(fila_datos) >= COLUMNAS_TIENDA["SUCURSAL_1"] else "0"
                    s3 = fila_datos[COLUMNAS_TIENDA["SUCURSAL_2"]-1] if len(fila_datos) >= COLUMNAS_TIENDA["SUCURSAL_2"] else "0"
                    s4 = fila_datos[COLUMNAS_TIENDA["SUCURSAL_3"]-1] if len(fila_datos) >= COLUMNAS_TIENDA["SUCURSAL_3"] else "0"
                    
                    st.info(f"**MI STORE:** {s1} | **SUC_1:** {s2} | **SUC_2:** {s3} | **SUC_3:** {s4}")
        ...
        with tabs[2]:
            ...
            with st.expander("📊 Reportes y Extractos Bancarios", expanded=True):
                ...
                if st.button("🌍 Generar Extracto Global de Hoy", type="primary"):
                    ...
                    ruta_h = os.path.join(sub_e, f"Extracto_{hoy}_{ahora}.html")
                    ruta_t = os.path.join(sub_e, f"Extracto_{hoy}_{ahora}.txt")
                    with open(ruta_h, "w", encoding="utf-8") as arc: arc.write(html_final)
                    with open(ruta_t, "w", encoding="utf-8") as arc: arc.write(t_txt)
