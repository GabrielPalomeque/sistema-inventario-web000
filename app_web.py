...
# ==============================================================================
# INICIALIZACIÓN DE VARIABLES DE SESIÓN (SESSION STATE) Y PERSISTENCIA DE URL
# ==============================================================================
# --- FUNCIÓN AGREGADA PARA QUE NO SE CIERRE SESIÓN AL BLOQUEAR LA PANTALLA ---
try:
    q_params = st.query_params
    u_param = q_params.get("u")
    c_param = q_params.get("c")
    t_param = q_params.get("t")
except AttributeError:
    q_params = st.experimental_get_query_params()
    u_param = q_params.get("u", [None])[0]
    c_param = q_params.get("c", [None])[0]
    t_param = q_params.get("t", [None])[0]

if 'logged_in' not in st.session_state:
    if u_param and c_param and t_param and t_param in COLUMNAS_TIENDA:
        st.session_state.logged_in = True
        st.session_state.usuario = u_param
        st.session_state.cargo = c_param
        st.session_state.tienda = t_param
        st.session_state.col_index = COLUMNAS_TIENDA[t_param]
    else:
                    ...
                    try:
                        ...
                        for fila in usuarios_db:
                            if str(fila['Usuario']).strip() == user_input.strip() and str(fila['Password']).strip() == pass_input.strip():
                                ...
                                # --- GUARDADO DE SESIÓN EN LA URL ---
                                try:
                                    st.query_params["u"] = st.session_state.usuario
                                    st.query_params["c"] = st.session_state.cargo
                                    st.query_params["t"] = st.session_state.tienda
                                except AttributeError:
                                    st.experimental_set_query_params(u=st.session_state.usuario, c=st.session_state.cargo, t=st.session_state.tienda)
...
# ==============================================================================
    ...
    if st.sidebar.button("🚪 Cerrar Sesión Segura", type="primary", use_container_width=True):
        ...
        # --- LIMPIAMOS LA SESIÓN DE LA URL AL SALIR ---
        try: 
